...
$(function () {
  ...
  */
   ...
   var process_fish=function(){

     $.ajax({
       url: $(this).attr('data-url')+`?month=${$("#month_select").val()}&sal=${$("#sal_select").val()}`,
       type: 'get',
       dataType: 'json',
       beforeSend:function(){
         $("#preloader").show();
       ...
       success: function (data) {
         if (data.form_is_valid) {
           $("#preloader").hide();
           $("#main-wrapper").show();
           swal({ title: "پایان پردازش", text: "<span style='color:#ff0000'>پردازش با موفقیت انجام شد<span>", html: !0 })
  ...
  $(".process_fish").on("click",process_fish)
